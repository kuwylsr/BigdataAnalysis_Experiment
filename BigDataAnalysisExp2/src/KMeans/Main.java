...
public class Main {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		String path2 = "hdfs://localhost:9000/user/hadoop/BDAnalysis/exp2/KMeans/Output/InitCluster/Cluster";
		RandomCluster c = new RandomCluster(k, path1, path2);
		String clusterList = c.InitCluster();
		//String clusterList = readFileFromHDFS("BDAnalysis/exp2/KMeans/Output/InitCluster/Cluster");
		int index = 0;
		while(index < iterationNum) {
			index++;
			Configuration conf = new Configuration();
			conf.set("clusterList", clusterList);
			conf.setDouble("varyLimit", limit);
			conf.setInt("flag", 0);
			
			Path inputFile = new Path(path1);
			Path refreshCluster = new Path("BDAnalysis/exp2/KMeans/Output/Cluster"+index);
		...
		Path inputFile = new Path(path1);
		Path outputFile = new Path("BDAnalysis/exp2/KMeans/Output/FinalResult");
		Path refreshCluster = new Path("BDAnalysis/exp2/KMeans/Output/Cluster"+index);
		...
		job1.setMapperClass(KMeans_Mapper.class);
